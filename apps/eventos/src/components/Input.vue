...
<template>
    ...
    <div v-else-if="props.option === 'select'" class="relative w-full">
        <select
            v-bind="$attrs"
            :class="{'border-red-500' : props.error }"
            class="peer m-0 block h-[58px] uppercase bg-white w-full rounded border focus-within:border-color-4 bg-transparent bg-clip-padding px-3 py-4 text-base font-normal leading-tight text-neutral-700 transition duration-200 ease-linear placeholder:text-transparent focus:border-primary focus:pb-[0.625rem] focus:pt-[1.625rem] focus:text-neutral-700 focus:outline-none peer-focus:text-primary [&:not(:placeholder-shown)]:pb-[0.625rem] [&:not(:placeholder-shown)]:pt-[1.625rem]"
            :value="props.modelValue" @input="$emit('update:modelValue', $event.target.value)"
        >
        <slot></slot>
        </select>
    ...
    <div v-else-if="props.option === 'date-picker'" class="relative w-full grid border rounded focus-within:border-color-4" :class="{'border-red-500' : props.error }">
        <label class="motion-reduce:transition-none uppercase text-color-4 px-3 pt-4">
            {{ props.title }}
        </label>
        <Date-Picker v-bind="$attrs" 
            :value="props.modelValue" 
            @input="$emit('update:modelValue', $event.target.value)" 
            class="focus:outline-none h-auto w-full px-3 bg-white" />
    </div>
