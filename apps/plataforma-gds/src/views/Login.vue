    ...
    <div class="grid md:grid-cols-2 bg-color-4 text-color-1 h-screen relative overflow-hidden bg-img bg-cover bg-center bg-no-repeat">
        <div class="flex justify-center items-center h-screen bg-color-1 z-10 rounded-br-[8rem]">
            <form @submit.prevent="handleLogin" class="grid gap-4 ">
                ...
                <Input v-model="auth.credentials.cui" icon="fas fa-id-card-clip" class="h-12" placeholder="Ingrese dpi" type="text" maxlength="13" minlength="13" required :error="auth.errors.hasOwnProperty('cui')" />
                <Input v-model="auth.credentials.password" icon="fas fa-key" class="h-12" placeholder="Password" type="password" maxlength="16" minlength="8" required :error="auth.errors.hasOwnProperty('password')" />
