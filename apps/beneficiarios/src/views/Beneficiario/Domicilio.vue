    ...
    <details class="border-t-2 py-4">
        ...
        <div class="grid lg:grid-cols-2 gap-4">
            <Input v-model="store.beneficiario.domicilio.departamento_id" option="select" title="*seleccione departamento" :error="store.errors.hasOwnProperty('domicilio.departamento_id')" required>
                <option value=""></option>
                <option v-for="departamento in catalogos.catalogo.departamentos" :value="departamento.id">{{ departamento.nombre }}</option>
            </Input>
            <div class="relative">
            ...
            <Input v-model="store.beneficiario.domicilio.grupo_habitacional_id" option="select" title="seleccione grupo" :error="store.errors.hasOwnProperty('domicilio.grupo_habitacional_id')">
                <option value=""></option>
                <option v-for="grupo_habitacional in catalogos.catalogo.grupos_habitacionales" :value="grupo_habitacional.id">{{ grupo_habitacional.nombre }}</option>
            </Input>
            <div class="relative">
