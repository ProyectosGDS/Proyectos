<script setup>
    ...
    import Actividad from './BeneficiarioActividad/Actividad.vue'
    ...
    const beneficiarios_actividad = computed(() => {
...
<template>
    <Card v-if="auth.checkPermission('ver inscripciones actividad')" class="bg-white p-4 xl:p-8">
        <div class="grid xl:grid-cols-2 xl:divide-x-2">
            <div class="space-y-4 xl:pr-8">
                ...
                <div class="flex items-center gap-2">
                    <Input @click="store.openActividades" v-model="store.actividad.actividad" option="label" title="*seleccione actividad" class="cursor-pointer" :error="store.errors.hasOwnProperty('actividad_id')" readonly />
                    <div class="grid gap-2">
                        <Icon @click="store.removeCurso" v-if="store.actividad.actividad" icon="fas fa-xmark" class="icon-button btn-danger" />
                        <Icon @click="inscripcion.fetch(store.actividad.id)" v-if="store.actividad.actividad" icon="fas fa-arrows-rotate" class="icon-button btn-secondary" :class="{'animate-spin' : inscripcion.loading.fetch}" />
                    </div>
                </div>
                ...
                <div class="flex justify-center gap-4">
                    <Tool-Tip v-if="!beneficiarios.nuevo_registro" message="Agregar beneficiario al actividad" class="-mt-6 text-color-4">
                        <Button @click="store.addBeneficiario()" icon="fas fa-plus" class="btn-primary" />
                    </Tool-Tip>
                    <Tool-Tip v-else message="Agregar beneficiario al actividad" class="-mt-6 text-color-4">
            ...
            <div class="xl:pl-8">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <Tool-Tip message="Excel" class="-mt-7 text-color-4">
                        <Icon v-if="auth.checkPermission('exportar excel inscripciones actividad')" @click="inscripcion.exportExcel" :icon="inscripcion.loading.excel ? 'fas fa-spinner' : 'fas fa-file-excel'" class="icon-button p-2 btn-success" :class="inscripcion.loading.excel ? 'animate-spin bg-gray-300 text-gray-500' : ''" :disabled="inscripcion.loading.excel" />
                    </Tool-Tip>
                    <Tool-Tip message="Pdf" class="-mt-7 text-color-4">
                        <Icon v-if="auth.checkPermission('exportar pdf inscripciones actividad')" @click="inscripcion.exportPdf" :icon="inscripcion.loading.pdf ? 'fas fa-spinner' : 'fas fa-file-pdf'" class="icon-button p-2 btn-danger" :class="inscripcion.loading.pdf ? 'animate-spin bg-gray-300 text-gray-500' : ''" :disabled="inscripcion.loading.pdf" />
                    </Tool-Tip>
                </div>
                <br>
                ...
                <div class="h-[40rem] overflow-y-auto">
                    <div class="grid gap-4">
                        <template v-for="(inscripcion,index) in beneficiarios_actividad">
                            <div class="flex gap-2 pr-4">
                                    ...
                                    <div class="grid xl:grid-cols-2 gap-2 text-xs uppercase">
                                        ...
                                        <span>
                                            <span class="flex items-center gap-1">
                                                ...
                                                FECHA INSCRIPCIÓN: 
                                                <span class="font-medium">{{ inscripcion.created_at ?? '' }}</span>
                                            </span>
                                        </span>
                                        <span>
                                            <span class="flex items-center gap-1">
                                ...
                                <div class="grid items-center">
                                    ...
                                    <template v-if="auth.checkPermission('desactivar inscripcion actividad')">
                ...
                <div v-if="inscripcion.beneficiarios.length" class="flex justify-center gap-4">
                    <Button v-if="auth.checkPermission('crear inscripcion actividad')" @click="inscripcion.store" text="Inscribir beneficiarios nuevos al actividad" icon="fas fa-plus" class="btn-primary" :loading="inscripcion.loading.store"/>
                </div>
            </div>
        </div>
    </Card>
    ...
    <Modal :open="store.modal.actividades" title="Actividades" icon="fas fa-book">
        <template #close>
