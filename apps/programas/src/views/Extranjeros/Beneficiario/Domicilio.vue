<script setup>
    ...
    watchEffect(() => {
        catalogos.getMunicipiosDepartamentoExtrajeros()
        catalogos.getGruposZonasExtranjeros()
    })
...
<template>
    <details :open="true" class="border-t-2 py-4">
        ...
        <div class="grid lg:grid-cols-2 gap-4">
            <Input v-model="store.extranjero.domicilio.departamento_id" option="select" title="*seleccione departamento" :error="store.errors.hasOwnProperty('domicilio.departamento_id')" required>
                <option value=""></option>
            ...
            <div class="relative">
                <Input v-model="store.extranjero.domicilio.municipio_id" option="select" title="*seleccione municipio" :error="store.errors.hasOwnProperty('domicilio.municipio_id')" required>
                    <option value=""></option>
            ...
            <Input v-model="store.extranjero.domicilio.zona_id" option="select" title="zonas" :error="store.errors.hasOwnProperty('domicilio.zona_id')">
                <option value=""></option>
                <option v-for="zona in catalogos.catalogo_beneficiario.zonas" :value="zona.id">{{ zona.descripcion }}</option>
            </Input>
            <Input @change="catalogos.getGruposZonas()" v-model="store.extranjero.domicilio.grupo_habitacional_id" option="select" title="seleccione grupo" :error="store.errors.hasOwnProperty('domicilio.grupo_habitacional_id')">
                <option value=""></option>
            ...
            <div class="relative">
                <Input v-model="store.extranjero.domicilio.grupo_zona_id" option="select" title="seleccione grupo habitacional" :error="store.errors.hasOwnProperty('domicilio.grupo_zona_id')">
                    <option value=""></option>
            ...
            <Input v-model="store.extranjero.domicilio.direccion" option="label" title="*dirección" maxlength="100" :error="store.errors.hasOwnProperty('domicilio.direccion')" required/>
        </div>
    </details>
</template>