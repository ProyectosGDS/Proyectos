...
<template>
    <details class="border-t-2 py-4">
        ...
        <div class="grid lg:grid-cols-2 gap-4">
            <Input v-model="store.extranjero.emergencia.cui" option="label" title="cui" maxlength="13" :error="store.errors.hasOwnProperty('emergencia.cui')"/>
            <Input v-model="store.extranjero.emergencia.fecha_nacimiento" option="label" title="*fecha nacimiento" type="date" :error="store.errors.hasOwnProperty('emergencia.fecha_nacimiento')"/>
            <Input v-model="store.extranjero.emergencia.parentesco_id" option="select" title="*seleccione parentesco" :error="store.errors.hasOwnProperty('emergencia.parentesco_id')">
                <option value=""></option>
                <option v-for="parentesco in catalogos.catalogo_beneficiario.parentescos" :value="parentesco.id">{{ parentesco.nombre }}</option>
            </Input>
            <Input v-model="store.extranjero.emergencia.nombres" option="label" title="*nombres" maxlength="150" :error="store.errors.hasOwnProperty('emergencia.nombres')"/>
            <Input v-model="store.extranjero.emergencia.apellidos" option="label" title="*apellidos" maxlength="150" :error="store.errors.hasOwnProperty('emergencia.apellidos')"/>
            <Input v-model="store.extranjero.emergencia.celular" option="label" title="*celular" maxlength="8" :error="store.errors.hasOwnProperty('emergencia.celular')"/>
            ...
            <div class="col-span-2">
                <Input v-model="store.extranjero.emergencia.direccion" option="label" title="direccion" maxlength="200" :error="store.errors.hasOwnProperty('emergencia.direccion')"/>
            </div>
        </div>
    </details>
</template>