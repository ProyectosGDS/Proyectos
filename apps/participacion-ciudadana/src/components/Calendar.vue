...
<template>
            ...
            <div class="w-full flex justify-between items-center">
                <div class="w-full flex justify-between items-center">
                    <div class="w-1/3 p-2 md:p-4">
                        <Date-Picker v-model="date" auto-apply close-on-scroll @update:modelValue="handleDate" month-picker>
                            <template #trigger>
                ...
                <div class="w-2/3 md:w-1/3 flex justify-end pr-2 md:pr-4">
                    <div class="flex space-x-2 md:space-x-5">
                        <Button @click="resetDate" text="Hoy" class="text-white bg-color-9" />
                    </div>
                </div>
            </div>
        </div>
        ...
        <div ref="calendarContainer" class="min-h-full min-w-full text-gray-800">
            <div class="w-full border grid grid-cols-7">
                ...
                <div v-for="day in daysInCurrentMonth" :key="day" class="h-14 w-full border align-top" @click="selectDay = day">
                    <div class="w-full h-full text-xs md:text-sm lg:text-base text-center transition-colors" 
                        ...
                        <div v-if="allTodaysEvent(day, events).length > 0" class="flex w-full justify-center items-center cursor-pointer" @click="openModal(day, allTodaysEvent(day, events))">
                            <div class="h-6 w-6 flex justify-center items-center text-xs bg-color-9 rounded-full shadow-sm">
                ...
                <div class="md:hidden col-span-7 flex justify-between items-center p-2">
                    <Icon @click="decrementMonth(1)"  icon="fas fa-angles-left" class="text-color-9 cursor-pointer text-xl hover:scale-110"/>
                    <Icon @click="incrementMonth(1)"  icon="fas fa-angles-right" class="text-color-9 cursor-pointer text-xl hover:scale-110"/>
                </div>
            </div>
        </div>
    </div>
    <div v-else>
        <div class="col-span-7">
            <div class="w-full flex justify-between items-center">
                <div class="w-full flex justify-between items-center">
                    <Icon icon="fas fa-angles-left" @click="decrementMonth()" class="text-color-9 cursor-pointer text-xl hover:scale-110 hidden md:block" />
                    <div class="w-1/3 p-2 md:p-4">
                        <Date-Picker v-model="date" month-picker auto-apply close-on-scroll @update:modelValue="handleDate" >
                            <template #trigger>
                                <div class="w-full inline-flex space-x-1 text-sm md:text-xl lg:text-2xl text-left font-bold md:font-semibold cursor-pointer">
                                    <span class="md:hidden">{{ shortMonthStr }}</span>
                                    <span class="hidden md:block">{{ monthStr }}</span>
                    ...
                    <Icon @click="incrementMonth()"  icon="fas fa-angles-right" class="text-color-9 cursor-pointer text-xl hover:scale-110 hidden md:block"/>
                </div>
                <!-- Date picker and date view -->
                <div class="w-2/3 md:w-1/3 flex justify-end pr-2 md:pr-4">
                    <div class="flex space-x-2 md:space-x-5">
                        <Button @click="resetDate" text="Hoy" class="bg-color-9 text-white" />
                    </div>
                </div>
            </div>
        </div>
    ...
    <Modal :open="openModalEvent" :title="evento.title" icon="fas fa-calendar-days" class="w-96">
        ...
        <div v-if="evento.hasOwnProperty('date')" class="grid gap-6">
            <Input type="text" icon="fas fa-bell" v-model="evento.title" class="h-12" disabled />
            <Input icon="fas fa-calendar-days" v-model="evento.fecha" class="h-12" disabled />
            <Input icon="fas fa-clock" v-model="evento.hora" class="h-12" disabled />
            <Input v-model="evento.description" option="text-area" title="descripción" rows="5" disabled>
            </Input>
        </div>
        <template #footer>
    ...
    <Modal v-if="allTodayEvent.hasOwnProperty('allEvents')" :open="openModalAllEvent" title="Eventos" icon="fas fa-calendar-days" class="w-96">
        <div class="grid gap-4">
            <details open v-for="event in allTodayEvent.allEvents" class="border rounded-lg p-5 ">
                <summary class="cursor-pointer text-color-9 font-medium">
                    {{ event.title }}
                </summary>
